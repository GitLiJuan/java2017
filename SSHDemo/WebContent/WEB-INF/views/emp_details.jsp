...
<body>
	<h1>员工信息操作平台</h1>
	<h3>欢迎${sessionScope.username }登录   <a href="${pageContext.request.contextPath }/router_report.action">报表查看</a></h3>
	<table border="1">
		<tr>
			<td>编号</td>
			<td>姓名</td>
		...
		<c:forEach items="${empListFromServer }" var="emp">
			<tr>
				...
				<td><a
					...
					<a href="${pageContext.request.contextPath }/emp_find_findById.action?type=update&empno=${emp.empno}">修改</a>
				</td>
			</tr>
		</c:forEach>
	</table>
	<hr />
	<a href="${pageContext.request.contextPath}/router_save.action ">新增员工</a>
	<hr />
	员工ID：
	<input type="number" value="1" id="empno" />
	<a href="${pageContext.request.contextPath }/emp_find_findById.action?empno="
		onclick="findByIdHandler()" id="findById">精确查询</a>
	<hr />
	<form action="${pageContext.request.contextPath}/emp_find_findByName.action"
		method="post">
